...
def Logistic_Regression(X, Y, learningRate=0.01, maxIter=100):
    ...
    ####### TODO: implement logistic regression (You should be able to implement this < 10 lines)
        J = np.zeros((N, D_plus_1))
        for i in range(N):
            x_n = X[:,i]
            error = -(Y[i] - sigmoid(np.dot(w.T, x_n)))*x_n

            J[i] = error

        avgError = np.average(J, axis=0).reshape((D_plus_1,1))
        w = w - (learningRate * avgError)
...
def Perceptron(X, Y, learningRate=0.01, maxIter=100):
    ...
    ####### TODO: implement perceptron (You should be able to implement this < 10 lines)
            x_n = X[:,n].reshape((D_plus_1,1))
            y_predict = np.sign(np.dot(w.T, x_n))
            if y_predict != Y[n]:
                w = w + (learningRate * (Y[n]*x_n))
